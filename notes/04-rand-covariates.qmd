...
- PS2 submitted
<!-- - PS3 submitted -->
<!-- - PS4 exercises will be posted, for Monday -->
...
\begin{center}
\begin{tabular}{cccc}
Precinct & Party & Canvass? & Enrol \%  \\ \hline
1 & Dem &  &  \\
2 & Dem &  &  \\
3 & Rep &  &  \\
4 & Rep &  &  \\ 
...
- If equal variances, make $n_{Tr} = n_{Co}$
- If unequal variances, more units to higher variance condition (to a point)
- If possible, define outcome s.t. $Cov(Y(1), Y(0)) < 0$
...
Does canvassing actually _change_ enrolment in precinct?  
(Or, just Party $\to$ Enrolment?)
...
&&& Enrol \% & Enrol \% & \alert{True Precinct} \\
...
(Then, random allocate \alert{within} groups.)
...
>- Bernoulli randomisation: $2^n = 2^4 = 16$
...
\large
>- Randomisation _should_ balance potential outcomes $\rightsquigarrow (Y(1), Y(0)) \independent T$, by design
...
>- If one unit attrits (no outcomes available), consider "pairwise deletion estimator" (PDE)
>- Unit-wise deletion biased under non-ignorable attrition
>- PDE _can_ also be biased, but 
>    - More efficient than unitwise
>    - Variance estimator unbiased
>    - Easier to interpet as causal
>- See, e.g., @fukumoto22
...
& \multicolumn{2}{c}{Gender} & \qquad & \multicolumn{2}{c}{Age} \\
...
\vspace{3mm} 
\hspace{10mm} \color{red}{$s_j>0$, (see numerator), bias toward Treatment}
...
\begin{center}
\includegraphics[angle=0,width=2.1in]{figs/03-balanceReald2pvalues} 
\includegraphics[angle=0,width=2.1in]{figs/03-balanceReald2pvaluesOutT}
\end{center}
...
>- Blocked DiM
>    - DiM in blocks, weight block by size
>- Or, if assignment probability for $i$ varies by block $j$, let $p_{ij} = m_{ij} / N_j$.  Weight each obs with \pause 
...
\pause 
If blocks used to randomise, incorporate into RI.
...
## Resources for Sequential Blocking

\Large

>- Simon Heuberger's thesis  
\url{https://www.linkedin.com/in/simon-heuberger-7147a936}
>- Charlotte Cavaille (Toulouse Sch of Econ)
>- Booking.com experiment 
>- Your contact at YouGov, etc.
>- Orthogonal designs (time blocks)
>    - VA opioid buyback experiment (OES)
>    - Fire inspectors experiment (The Lab @ DC)
>    - @moohab26
...
\begin{center}
Next:\\
Clustered Designs, Regression and Experiments\\
PS3 due today
