...
>- Perform design-based inference for randomised experiments
>- Create and analyze variety of randomised designs, including for blocked, 
...
## Example: Canvassing and Programme Enrolment

\pause 

\Large

"Canvassing": a systematic programme where agents talk with residents (knock on doors) at the residents' home.
...
Suppose we ask, ``Would a canvassing policy increase enrolment in a health insurance programme?\text{''} \pause 
...
>- What fraction enrol under canvassing vs. no canvassing?  
>- $\frac{2}{2} - \frac{0}{2} = 1$
>- (For each person canvassed, expect 1 more enrolment.)

>- Did the policy "work" (cause more enrolment)?

## Motivating Example: Canvassing and Enrolment
...
Would canvassing actually _change_ anyone's enrolment? \newline \pause 
...
\begin{center}
\begin{tabular}{ccccc}
 & & Would Enrol if & Would Enrol if  & \\
Citizen & Canvass? & Canvass? & No Canvass? & Enrol \\ \hline
...
\onslide<8->{What fraction enrol under canvass vs. no canvass? \\} 
\onslide<9->{What fraction change enrolment due to canvass?\\} 
...
What fraction enrol under canvass vs. no canvass? \newline  \pause 
...
\item[] (But, those who choose Canvass will Enrol anyway) \pause 
...
>* _parameter_: unknown numeric value characterising feature of
...
>- Intent-to-treat effect (as-assigned)
	...
	\end{eqnarray*} 
>- Causal effect of _being assigned_ (ignores receipt)
>- As-treated difference
	\begin{eqnarray*}
	ASTRE &=& (\overline{Y_1} | D = 1) - (\overline{Y_0} | D = 0)
	\end{eqnarray*}
>- If selection into $D$ is confounded, likely *not* causal effect (ignores assignment)
>- Per protocol difference
\begin{eqnarray*}
PP &=& \big(\overline{Y_1} \mid T = 1, D = 1\big) -
  \big(\overline{Y_0} \mid T = 0, D = 0\big)
\end{eqnarray*}
>- If noncompliance, still confounded
...
\begin{center}
Next:\\
Randomised Experiments, Estimation, and Inference\\ 
\pause 
\vspace{5mm}
